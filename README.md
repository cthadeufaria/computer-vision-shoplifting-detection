...
2. Put the dataset under the `dataset/` folder following the structure listed below. Create at least one of the two folders: `Shoplifting/` and/or `Shopping/` with mp4 videos inside. Note: Use the Anomaly_Train.txt in this repository if using the `Shopping/` folder.
3. Create and populate the tabular dataset (preprocessing) by running:
...
Expected dataset directory layout for shoplifting detection:
